...
AudioVisualizer::AudioVisualizer() {
    ...
    for (int ranges = 0; ranges < 64; ranges++) {
        sf::RectangleShape freqRangeRect(sf::Vector2f(4, 0));
...
std::vector<sf::RectangleShape>& AudioVisualizer::getFreqRangeRects() {
    return freqRangeRects;
}

sf::RectangleShape& AudioVisualizer::getBackgroundRect() {
    return backgroundRect;
}

std::vector <sf::CircleShape>& AudioVisualizer::getAmplitudeParticles() {
    return amplitudeParticles;
}

void AudioVisualizer::setAmplitudeVisualizationVector(std::vector<double> amplitudeVisualizationVector) {
    this->amplitudeVisualizationVector = amplitudeVisualizationVector;
}

void AudioVisualizer::update(std::vector< std::vector <double> >& frequencyVisualizationVector, double songPlayingOffset) {
